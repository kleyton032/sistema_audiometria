-- ============================================================================
-- SCRIPT PARA ADICIONAR OBJETIVOS DE BRAILLE NA PSICOPEDAGOGIA
...
DECLARE
    v_max_id NUMBER;
    v_count NUMBER := 0;
    v_id_psicopedagogia NUMBER;
    v_duplicado NUMBER;
BEGIN
    -- 0. Descobrir ou criar a Especialidade "PSICOPEDAGOGIA"
    BEGIN
        SELECT ID_ESPECIALIDADE INTO v_id_psicopedagogia
        FROM FAV_TB_ESP_OBJETIVO_CERIV
        WHERE UPPER(DS_ESPECIALIDADE) = 'PSICOPEDAGOGIA'
        AND ROWNUM = 1;
    EXCEPTION
        WHEN NO_DATA_FOUND THEN
            SELECT NVL(MAX(ID_ESPECIALIDADE), 0) + 1 INTO v_id_psicopedagogia 
            FROM FAV_TB_ESP_OBJETIVO_CERIV;
            
            INSERT INTO FAV_TB_ESP_OBJETIVO_CERIV (ID_ESPECIALIDADE, DS_ESPECIALIDADE)
            VALUES (v_id_psicopedagogia, 'PSICOPEDAGOGIA');
    END;

    -- Descobrir o ID máximo para continuar a sequência
    SELECT NVL(MAX(ID_OBJETIVO), 0) INTO v_max_id FROM FAV_TB_OBJETIVO_CERIV;
    
    -- Inserir os objetivos de Braille na Psicopedagogia, garantindo que não crie duplicatas exatas
    FOR rec IN (
        ...
        -- Verifica se esse objetivo já existe na Psicopedagogia (para não duplicar se rodar duas vezes)
        SELECT COUNT(*) INTO v_duplicado
        FROM FAV_TB_OBJETIVO_CERIV
        WHERE ID_ESPECIALIDADE = v_id_psicopedagogia
        AND UPPER(TRIM(DS_OBJETIVO)) = UPPER(TRIM(rec.objetivo));

        IF v_duplicado = 0 THEN
            v_count := v_count + 1;
            INSERT INTO FAV_TB_OBJETIVO_CERIV (ID_OBJETIVO, ID_ESPECIALIDADE, DS_OBJETIVO, IC_ATIVO)
            VALUES (v_max_id + v_count, v_id_psicopedagogia, rec.objetivo, 'S');
        END IF;
    END LOOP;

    DBMS_OUTPUT.PUT_LINE('Foram adicionados ' || v_count || ' objetivos de braille na especialidade de Psicopedagogia.');
END;
/
COMMIT;
